...
@section('content')
            ...
            <div class="card my-0">
                <div
                    ...
                    <div class="d-flex flex-column">
                        <h3 class="p-0 m-0 mb-1 fw-semibold">Edit Partner</h3>
                        <p class="p-0 m-0 fw-medium text-muted">Update this partner.</p>
                    </div>
    ...
    <div class="row">
        <div class="col">
            <div class="card my-0">
                <div class="card-body">
                    <form action="{{ route('dashboard.master-data.partners.update', $partner->id) }}" autocomplete="off"
                        method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                            <input type="file" name="logo_file" id="imageInput" class="d-none @error('logo_file') is-invalid @enderror" accept="image/*">
                            <img src="{{ $partner->logo_url ?? asset('static/img/no-image-placeholder.svg') }}" id="imagePreview" alt="Preview Image" class="w-100 rounded object-fit-cover border" style="max-width: 300px; height: 200px; object-position: center; cursor: pointer;">
                        ...
                        <div class="form-floating mb-3">
                            <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" id="floatingInputName"
                                placeholder="Name" autocomplete="off" value="{{ old('name', $partner->name) }}" required>
                        ...
                        <div class="form-floating mb-3">
                            <input type="text" name="website_url" class="form-control @error('website_url') is-invalid @enderror" id="floatingInputWebsite"
                                placeholder="Website" autocomplete="off" value="{{ old('website_url', $partner->website_url) }}">
                        ...
                        <div class="form-floating mb-3">
                            <textarea name="description" class="form-control @error('description') is-invalid @enderror" id="floatingInputDescription"
                                placeholder="Description" autocomplete="off"  data-lenis-prevent>{{ old('description', $partner->description) }}</textarea>
                        ...
                        <div class="form-floating mb-3">
                            <select name="is_featured" id="floatingInputOrder" class="form-select @error('is_featured') is-invalid @enderror" required>
                                <option value="1" {{ old('is_featured', $partner->is_featured) == '1' ? 'selected' : '' }}>
                                    Visible
                                </option>
                                <option value="0" {{ old('is_featured', $partner->is_featured) == '0' ? 'selected' : '' }}>
                                    Hidden
                                </option>
                            </select>
                        ...
                        <div class="form-floating mb-3">
                            <select name="order" id="floatingInputOrder"
                                ...
                                <option value="" disabled {{ old('order', $partner->order) ? '' : 'selected' }}>Select
                                    Order</option>
                                @for ($i = 1; $i <= $allowedMaxOrder; $i++)
                                    <option value="{{ $i }}"
                                        {{ old('order', $partner->order) == $i ? 'selected' : '' }}>
                                        {{ $i }}
                                    </option>
                                @endfor
                            </select>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const originalImage = "{{ $partner->logo_url ?? asset('static/img/no-image-placeholder.svg') }}";
