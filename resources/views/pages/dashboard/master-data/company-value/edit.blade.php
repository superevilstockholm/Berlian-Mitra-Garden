...
@section('content')
            ...
            <div class="card my-0">
                <div
                    ...
                    <div class="d-flex flex-column">
                        <h3 class="p-0 m-0 mb-1 fw-semibold">Edit Company Value</h3>
                        <p class="p-0 m-0 fw-medium text-muted">Update this company value.</p>
                    </div>
    ...
    <div class="row">
        <div class="col">
            <div class="card my-0">
                <div class="card-body">
                    <form action="{{ route('dashboard.master-data.company-values.update', $company_value->id) }}" autocomplete="off"
                        method="POST">
                        @csrf
                        @method('PUT')
                        <div class="form-floating mb-3">
                            <input type="text" name="title" class="form-control @error('title') is-invalid @enderror" id="floatingInputTitle"
                                placeholder="Title" autocomplete="off" value="{{ old('title', $company_value->title) }}" required>
                        ...
                        <div class="form-floating mb-3">
                            <textarea name="description" class="form-control @error('description') is-invalid @enderror" id="floatingInputDescription"
                                placeholder="Description" autocomplete="off" required data-lenis-prevent>{{ old('description', $company_value->description) }}</textarea>
                        ...
                        <div class="form-floating mb-3">
                            <select name="order" id="floatingInputOrder"
                                ...
                                <option value="" disabled {{ old('order', $company_value->order) ? '' : 'selected' }}>Select
                                    Order</option>
                                @for ($i = 1; $i <= $allowedMaxOrder; $i++)
                                    <option value="{{ $i }}"
                                        {{ old('order', $company_value->order) == $i ? 'selected' : '' }}>
                                        {{ $i }}
                                    </option>
                                @endfor
                            </select>
                        ...
                        <div class="d-flex justify-content-end">
                            <button type="submit" class="btn btn-primary d-flex align-items-center gap-2 justify-content-center px-4 rounded-pill">
                                <i class="ti ti-device-floppy me-1"></i> Update Company Value
                            </button>
                        </div>
                    </form>
                </div>
