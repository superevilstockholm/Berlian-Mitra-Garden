...
@section('title', 'Company Value Management')
@section('content')
    @php
            ...
            <div class="card my-0">
                <div
                    ...
                    <div class="d-flex flex-column">
                        <h3 class="p-0 m-0 mb-1 fw-semibold">Company Value Records</h3>
                        <p class="p-0 m-0 fw-medium text-muted">Manage company value records.</p>
                    </div>
                    <div class="d-flex align-items-center">
                        <a href="{{ route('dashboard.master-data.company-values.create') }}"
                            class="btn btn-sm btn-primary px-4 rounded-pill m-0">
                            <i class="ti ti-plus me-1"></i> Create Company Value
                        </a>
                    </div>
                </div>
            </div>
    ...
    <div class="row">
        <div class="col">
            <div class="card my-0">
                <div class="card-body">
                    <form method="GET" action="{{ route('dashboard.master-data.company-values.index') }}" id="filterForm">
                        <div
                            ...
                            <div class="text-muted small">
                                @if ($company_values instanceof LengthAwarePaginator)
                                    Showing {{ $company_values->firstItem() }} to {{ $company_values->lastItem() }} of
                                    {{ $company_values->total() }} entries
                                @else
                                    Showing {{ $company_values->count() }} entries
                                @endif
                            </div>
                        </div>
                    ...
                    <div class="table-responsive @if (!($company_values instanceof LengthAwarePaginator && $company_values->hasPages())) mb-0 @else mb-3 @endif">
                        <table class="table table-striped table-hover align-middle mb-0">
                            ...
                            <tbody>
                                @forelse ($company_values as $index => $company_value)
                                    <tr>
                                        <td class="text-center">
                                            @if ($company_values instanceof LengthAwarePaginator)
                                                {{ $company_values->firstItem() + $loop->index }}
                                            @else
                                                {{ $loop->iteration }}
                                            @endif
                                        </td>
                                        <td>{{ $company_value->title ?? '-' }}</td>
                                        <td>{{ $company_value->description ? Str::limit($company_value->description, 60) : '-' }}</td>
                                        <td>{{ $company_value->created_at?->format('d M Y H:i') }}</td>
                                        <td class="text-center">
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-end">
                                                    <a class="dropdown-item d-flex align-items-center gap-2"
                                                        href="{{ route('dashboard.master-data.company-values.show', $company_value->id) }}">
                                                        <i class="ti ti-eye me-1"></i> View Details
                                                    </a>
                                                    <a class="dropdown-item d-flex align-items-center gap-2"
                                                        href="{{ route('dashboard.master-data.company-values.edit', $company_value->id) }}">
                                                        <i class="ti ti-pencil me-1"></i> Edit
                                                    </a>
                                                    <form id="form-delete-{{ $company_value->id }}"
                                                        action="{{ route('dashboard.master-data.company-values.destroy', $company_value->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="button" class="dropdown-item d-flex align-items-center gap-2 text-danger btn-delete"
                                                            data-id="{{ $company_value->id }}"
                                    ...
                                    <tr>
                                        <td colspan="5" class="text-center">
                                            <div class="alert alert-warning my-2" role="alert">
                                                No company value records found for the selected filters.
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
                    ...
                    @if ($company_values instanceof LengthAwarePaginator && $company_values->hasPages())
                        <div class="overflow-x-auto mt-0 py-1">
                            <div class="d-flex justify-content-center d-md-block w-100 px-3">
                                {{ $company_values->onEachSide(1)->links('vendor.pagination.bootstrap-5') }}
                            </div>
                        </div>
                    @endif
                </div>
